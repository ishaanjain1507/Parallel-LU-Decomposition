...
vector<double> sent_buffer, received_buffer;
...
public:
    ...
    ProcessorNode(int id, int n) {
        ...
        buffer.resize(n);
        localCols.resize(n);
        for (int i=0; i<n; i++) localCols[i].resize(r);
        l = 0;
    ...
    void Send() {
        ...
        cout<<"Going ouot of send "<<proc_id<<endl;
        return;
    }
    ...
    void Receive() {
        ...
        while(!this->received.load()) {} //spin until it receives the buffer from previous thread
    ...
    void Prep(int k) {
        ...
        for (int i=0; i<mat_size; i++) this->buffer[i] = localCols[i][l];
...
public:
    ...
    void DivideAmongThreads() {
        ...
        for (int col=0; col<mat_size; col++) {
            ...
            for (int row=0; row<mat_size; row++) {
                temp->localCols[row][temp->l] = matrix[row][col];
            }
            temp->l++;
        }

        //setting back the local index for columns to 0
        for (int i=0; i<numprocs; i++) procs[i]->l = 0;
        return;
    }
    ...
    void generateOutput() {
        ...
        fstream output;
        output.open("LU-pipeline-Output.txt", ios::out);
...
int main() {
    ...
    pthread_barrier_init(&b, NULL, n_procs);
	  sent_buffer.resize(mat_size);
    received_buffer.resize(mat_size);
